...
/**
 * ============================================================ CONCEPT: String
 * → Stream<Character> → Frequency Map
 * ============================================================ PROBLEM: Count
 * frequency of each character in a string using Streams.
 *
 * KEY LEARNING: How to convert a String into a stream of characters.
 *
 * THREE WAYS to stream characters from a String: 1. str.chars() → IntStream
 * (code points as int) 2. str.chars().mapToObj(c -> (char) c) →
 * Stream<Character> ← most common 3. Stream.of(str.split("")) → Stream<String>
 * (each element is 1-char string)
 *
 * INTERVIEW ANGLE: Interviewers often ask: "How do you process each character
 * of a string with Streams?" Answer: str.chars().mapToObj(c -> (char) c) —
 * explain the IntStream → Character conversion.
 *
 * THREE WAYS to print a Map (all shown below): 1. forEach with lambda → full
 * control over format 2. entrySet().forEach() → method reference style 3.
 * forEach(System.out::println) → most concise
 * ============================================================
 */
