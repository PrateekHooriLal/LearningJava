...
/**
 * ============================================================ CONCEPT: Stream
 * Creation + Sorting + Finding Max Element
 * ============================================================ TOPICS COVERED:
 * 1. Stream creation from array and List 2. Natural sort via Comparable
 * (Arrays.sort, Collections.sort) 3. Custom sort via Comparator
 * (stream().sorted()) 4. Finding max/min with Comparator 5. Optional — safe
 * container for nullable results
 *
 * HOW TO CREATE A STREAM (4 ways): 1. Stream.of(array) → from an array 2.
 * list.stream() → from a Collection 3. Arrays.stream(array) → from array (can
 * specify range) 4. Stream.of(a, b, c) → from individual elements (varargs)
 *
 * INTERVIEW ANGLE: "How do you sort a list of objects by salary descending?"
 * "How do you find the employee with the maximum salary?" These are standard
 * warm-up questions at any Java interview.
 *
 * SORT METHODS — KNOW ALL THREE: Arrays.sort(arr) → sorts array in-place, uses
 * compareTo (Comparable) Collections.sort(list) → sorts list in-place, uses
 * compareTo (Comparable) list.stream().sorted(comp) → returns NEW sorted stream
 * (non-destructive)
 * ============================================================
 */
public class StreamCreation {

	// Static array of Employee objects — used to demonstrate array → stream
	// conversion
	public static Employee[] arrayOfEmps = { new Employee(1, "Jeff Bezos", 300000.0, "HR"),
			...
			new Employee(-1, "Robert", 140, "HR"), new Employee(43, "Robert", 17000.0, "DEV") };
	...
	public static void main(String[] args) {
		...
		// stream().sorted(comparator) → returns a NEW sorted stream, original list
		// unchanged
		// Comparator.comparing(Employee::getSalary) builds a Comparator from a key
		// extractor
		System.out.println("\nStream sorted ascending by salary:");
		sortEmpSalaryAesc(emplist.stream(), "Salary").forEach(e -> {
			e.printEmp();
		});
		...
		sortEmpSalaryDesc(emplist.stream()).forEach(e -> {
			e.printEmp();
		});

		// ---- Find Max Salary ----
		// stream().max(comparator) → returns Optional<T>
		// Optional because if stream is empty, there is no max → returns
		// Optional.empty()
		// NEVER call .get() directly — use .orElseThrow() or .orElse(default)
		System.out.print("\nEmployee with max salary: ");
		findMaxSalary(emplist).ifPresent(Employee::printEmp);

		System.out.print("\n Employee Department wise with 2nd highest salary: \n");

		find2ndHighestEmpBySalary(emplist).forEach((dept, empOpt) -> System.out.println("Department= " + dept + " : "
				+ empOpt.map(Employee::getName).orElse("N/A") + " : " + empOpt.map(Employee::getSalary).orElse(0.0)));

		System.out.print("\nEmployee Department wise with 2nd highest salary: \n");

		findSecHighestTreeSet(emplist).forEach((dept, empOpt) -> System.out.println("Department= " + dept + " : "
				+ empOpt.map(Employee::getName).orElse("N/A") + " : " + empOpt.map(Employee::getSalary).orElse(0.0)));

		System.out.println("\n--- Average Salary Department-Wise ---");
		findAvgSalaryDeptWise(emplist).forEach((dept, avgSalary) -> System.out
				.printf("Department: %-10s | Average Salary: $%,.2f%n", dept, avgSalary));

		System.out.println("\n findAvgSalaryDeptWise =\n" + findAvgSalaryDeptWise(emplist));
	}
	...
	public static List<Employee> sortEmpSalaryDesc(Stream<Employee> s) {
		return s.sorted(Comparator.comparing(Employee::getSalary)).collect(Collectors.toList());
	}
	...
	public static Map<String, Optional<Employee>> find2ndHighestEmpBySalary(List<Employee> emplist) {
		...
		return secondHighestSalaryByDept;
	}

	public static Map<String, Optional<Employee>> findSecHighestTreeSet(List<Employee> emplist) {

		return emplist.stream().collect(Collectors.groupingBy(Employee::getDepartment, Collectors.collectingAndThen(
				Collectors.toCollection(() -> new TreeSet<Employee>(
						Comparator.comparingDouble(Employee::getSalary).reversed().thenComparing(Employee::getId))),
				set -> set.stream().skip(1).findFirst())));
	}

	public static Map<String, Double> findAvgSalaryDeptWise(List<Employee> emplist) {
